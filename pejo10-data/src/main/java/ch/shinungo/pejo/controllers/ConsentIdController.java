...
public class ConsentIdController {
	...
	public String getConsentId(@ModelAttribute UserForm form, Model model) throws JsonProcessingException {
		...
		log.debug("==> Für User  : " + user.getName());
		...
		log.debug("==> Bank Link: " + respEntity.getBody().getLinks().getScaRedirect().getHref());
		log.debug("==> Es folgt: TanVerfication() ");
	...
	public String getAccounts(@RequestParam(value = "consentId", required = true) String consentId, Model model)
		...
		Account seletedAccountInfo;

		HttpHeaders headers = prepareHeaders();
		headers.set("Consent-ID", consentId);
		
		log.debug("==> Consent-ID zu 'Header-Informationen' hinzugefügt");
		...
		log.debug("==> Account URL = BANK_URL + ConsentID");

		for (Account currentAccount : respEntity.getBody().getAccounts()) {

			seletedAccountInfo = getAccountDetails(currentAccount, consentId);
			log.debug("==> account Details: " + seletedAccountInfo);

			List<Balance> getbalancesFromAccount = getbalancesFromAccount(seletedAccountInfo, consentId);
			currentAccount.setBalances(getbalancesFromAccount);

			List<Booked> getBookedTransactions = getTransactions(seletedAccountInfo, consentId);
...
//		log.debug("==> Booked: " + respEntity.getBody().getTransactions().getBooked());
